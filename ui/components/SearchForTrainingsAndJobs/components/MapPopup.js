                ...
                <ul className="c-mapbox-list">
                  {list.map((job, idx) => (
                    <li className={`c-mapbox-list-item ${idx === list.length - 1 ? "is-last" : ""} ${idx === 0 ? "is-first" : ""}`} key={idx}>
                      <button className={`c-mapboxpopup--link gtmSavoirPlus gtm${capitalizeFirstLetter(job.ideaType)} gtmMap`} aria-label="Accès au détail de l'opportunité" onClick={() => openItemDetail(job)}>
                        {job.title}
                      </button>
